...
- [ ] **Rich editor increment 2** — port the M3 power features onto CodeMirror so the `richEditor` flag can become the default: slash commands, snippet expand, find/replace, typewriter scroll, typing sounds. Then flip default + retire the textarea path (or keep as a "classic" pref).
- [ ] **Bundle headroom (now urgent — ~0.2 KB left)** — markdown-it + DOMPurify (~56 KB) stay in the initial bundle even after PreviewPane/ExportMenu went lazy (Rollup hoists the shared dep). Add a `manualChunks` rule in vite.config to force markdown-it+dompurify into their own async chunk. Do this before ANY initial-bundle feature.
