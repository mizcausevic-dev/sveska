...
- [ ] **Bundle headroom** — initial JS is 179.62/180 KB after image paste. Before the next initial-bundle feature, lazy-load the markdown renderer (markdown-it + DOMPurify, ~56 KB) behind `import()` so plain-text users don't pay for it. Buys back the budget.
