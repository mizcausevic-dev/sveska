...
- **Canonical (current, post-migration)**: `https://sveska.pages.dev` — direct CF Pages URL. Sveska README + index.html canonical + sitemap + OG tags all point here.
- **Branded URL `sveska.studio`**: 301-redirects to `sveska.pages.dev` via Hostinger's URL forwarder (A record `@ 2.57.91.91`). Custom-domain attach to CF Pages is deferred because CF now requires Cloudflare nameservers for new Pages domains — staying on Hostinger DNS for now.
...
- **Custom domain attach deferred (2026-05-17)**: CF Pages now requires Cloudflare nameservers for new custom domains — the "external DNS / CNAME setup" path was removed for new Pages projects. To avoid touching nameservers (the prior NS-drift incident on this same domain made it risky), shipped with `sveska.pages.dev` as canonical and a Hostinger URL forwarder (`A @ 2.57.91.91` → `https://sveska.pages.dev` 301). README + index.html canonical + sitemap + OG URL all flipped to `sveska.pages.dev`. Brand-text mentions ("Sveska · sveska.studio" in PDF/image exports) kept as branding — work fine through the 301. **Future**: if traffic justifies it, do the CF NS migration and reverse the canonical flip; otherwise the redirect setup is steady-state.
